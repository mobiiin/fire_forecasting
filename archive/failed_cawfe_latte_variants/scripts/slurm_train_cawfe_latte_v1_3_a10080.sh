#!/usr/bin/env bash
#SBATCH --job-name=cawfe_latte_v1_3
...
#SBATCH --output=artifacts/logs/slurm/cawfe_latte_v1_3_%j.out
#SBATCH --error=artifacts/logs/slurm/cawfe_latte_v1_3_%j.err

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/home/mhabibp/fire_forecasting}"
CONFIG_PATH="${1:-${CONFIG_PATH:-configs/experiments/cawfe_latte_v1_3.yaml}}"
RUN_NAME="${RUN_NAME:-cawfe_latte_v1_3_slurm${SLURM_JOB_ID:-local}}"
...
echo "========== CAWFE-Latte v1.3 deep sanity check =========="
...
echo "========== CAWFE-Latte v1.3 one-batch smoke train/val =========="
srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL \
  /usr/bin/env PYTHONPATH="${PYTHONPATH}" "${PYTHON_BIN}" \
...
EXPECTED_RUN_DIR="${REPO_ROOT}/artifacts/runs/cawfe_latte_v1_3/${RUN_NAME}"
...
EXPECTED_BEST_CHECKPOINT="${REPO_ROOT}/artifacts/runs/cawfe_latte_v1_3/${RUN_NAME}/checkpoints/best_model.pt"
BEST_CHECKPOINT="${EXPECTED_BEST_CHECKPOINT}"
if [[ ! -f "${BEST_CHECKPOINT}" ]]; then
  RESOLVED_BEST_CHECKPOINT="$("${PYTHON_BIN}" -c 'from pathlib import Path; import sys; root=Path(sys.argv[1]); prefix=sys.argv[2]; matches=sorted(root.glob(prefix + "*/checkpoints/best_model.pt"), key=lambda p: p.stat().st_mtime, reverse=True); print(matches[0] if matches else "")' "${REPO_ROOT}/artifacts/runs/cawfe_latte_v1_3" "${RUN_NAME}")"
...
if [[ "${RUN_QUALITATIVE_EVAL}" == "1" ]]; then
  echo "========== CAWFE-Latte v1.3 qualitative evaluation =========="
  ...
  srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL \
    ...
    --mode qualitative \
    --split "${EVAL_SPLIT}" \
    --model_architecture cawfe_latte_v1_3 \
