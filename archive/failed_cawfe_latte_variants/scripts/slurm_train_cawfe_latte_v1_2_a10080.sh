...
EXPECTED_RUN_DIR="${REPO_ROOT}/artifacts/runs/cawfe_latte_v1_2/${RUN_NAME}"
...
EXPECTED_BEST_CHECKPOINT="${REPO_ROOT}/artifacts/runs/cawfe_latte_v1_2/${RUN_NAME}/checkpoints/best_model.pt"
BEST_CHECKPOINT="${EXPECTED_BEST_CHECKPOINT}"
if [[ ! -f "${BEST_CHECKPOINT}" ]]; then
  RESOLVED_BEST_CHECKPOINT="$("${PYTHON_BIN}" -c 'from pathlib import Path; import sys; root=Path(sys.argv[1]); prefix=sys.argv[2]; matches=sorted(root.glob(prefix + "*/checkpoints/best_model.pt"), key=lambda p: p.stat().st_mtime, reverse=True); print(matches[0] if matches else "")' "${REPO_ROOT}/artifacts/runs/cawfe_latte_v1_2" "${RUN_NAME}")"
