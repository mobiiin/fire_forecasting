"""Smoke-test the ST-Mamba-Lite model with a fake batch."""
...
def build_argument_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(description="Run a fake-batch smoke test for ST-Mamba-Lite.")
...
def main() -> None:
	...
	backend_used = getattr(model, "mamba_backend_used", "unknown")
	...
	print(f"mamba_backend_used: {backend_used}")
	print(f"scan_mode: {section.get('scan_mode', 'route_pair')}")
	print(f"scan_routes: {scan_routes}")
	if torch.cuda.is_available():
