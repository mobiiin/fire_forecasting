"""Run the multitask linear-extrapolation baseline on one dataset split."""
...
from src.baselines import evaluate_baseline, predict_linear_extrapolation_for_sample


def build_argument_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(description="Evaluate the linear-extrapolation baseline.")
...
def main() -> None:
	...
	if output_csv is None:
		output_csv = Path(f"artifacts/logs/linear_extrapolation_baseline_{args.split}.csv")
	result = evaluate_baseline(
		config_path=args.config,
		split=args.split,
		method_name="linear_extrapolation",
		predict_fn=predict_linear_extrapolation_for_sample,
		mode=args.mode,
