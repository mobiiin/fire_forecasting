...
#SBATCH --output=artifacts/logs/slurm_precompute_cache_with_config_%j.out
#SBATCH --error=artifacts/logs/slurm_precompute_cache_with_config_%j.err
...
srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL /usr/bin/env PYTHONPATH="${PYTHONPATH}" \
  "${PYTHON_BIN}" scripts/precompute_patch_cache.py --config "${CONFIG_PATH}" --split "${SPLIT}"
