"""Evaluate a full CAWFE-Latte checkpoint."""
...
def _config_override() -> dict:
	return {
		"model": {"architecture": "cawfe_latte", "name": "cawfe_latte"},
		"checkpoint": {
...
def build_argument_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(description="Evaluate a full CAWFE-Latte checkpoint.")
...
def main() -> None:
	...
	aggregate_csv = Path("artifacts/logs/cawfe_latte_test_metrics.csv").resolve()
	per_fire_csv = Path("artifacts/logs/cawfe_latte_test_metrics_by_fire.csv").resolve()
	_write_csv(aggregate_csv, [{"split": result["split"], "num_samples": result["num_samples"], **result["aggregate_results"]}])
	_write_csv(
		per_fire_csv,
		[{"split": result["split"], "dataset_name": dataset_name, **metrics} for dataset_name, metrics in sorted(result["per_dataset_results"].items())],
	)
