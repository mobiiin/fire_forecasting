...
#SBATCH --output=artifacts/logs/slurm_train_st_mamba_lite_%j.out
#SBATCH --error=artifacts/logs/slurm_train_st_mamba_lite_%j.err
...
RUN_NAME="st_mamba_lite_${SLURM_JOB_ID:-local}"
echo "Run name: ${RUN_NAME}"

echo "========== Training ST-Mamba-lite =========="
srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL /usr/bin/env PYTHONPATH="${PYTHONPATH}" "${PYTHON_BIN}" -m scripts.train_st_mamba_lite \
