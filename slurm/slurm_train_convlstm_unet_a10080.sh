...
RUN_NAME="convlstm_unet_${SLURM_JOB_ID:-local}"
echo "Run name: ${RUN_NAME}"

echo "========== Training ConvLSTM U-Net =========="
srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL /usr/bin/env PYTHONPATH="${PYTHONPATH}" "${PYTHON_BIN}" -m scripts.train_convlstm_unet \
