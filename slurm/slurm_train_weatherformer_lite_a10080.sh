...
RUN_NAME="weatherformer_lite_${SLURM_JOB_ID:-local}"
echo "Run name: ${RUN_NAME}"

echo "========== Training WeatherFormer-lite =========="
srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL /usr/bin/env PYTHONPATH="${PYTHONPATH}" "${PYTHON_BIN}" -m scripts.train_weatherformer_lite \
