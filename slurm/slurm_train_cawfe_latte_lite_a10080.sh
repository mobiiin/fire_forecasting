...
#SBATCH --error=artifacts/logs/slurm_train_cawfe_latte_lite_%j.err
...
RUN_NAME="cawfe_latte_lite_${SLURM_JOB_ID:-local}"
echo "Run name: ${RUN_NAME}"

echo "========== Training CAWFE-Latte-Lite =========="
srun --ntasks=1 --chdir="${REPO_ROOT}" --export=ALL /usr/bin/env PYTHONPATH="${PYTHONPATH}" "${PYTHON_BIN}" -m scripts.train_cawfe_latte_lite \
